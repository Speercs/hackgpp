...
info.showAttack = function () {
    ...
    _.forEach(Game.rooms, room => {
        ...
        myRoom.attackEvents.forEach(event => {
            ...
            if (target && (target.my || target.structureType == 'constructedWall')) {
                ...
                if (username && username !== 'Invader' && username !== 'Power Bank' && username !== settings.username) {
                    ...
                    if (myRoom.my) {
                        myRoom.room.memory.underAttack = true; myRoom.room.memory.underAttackTick = gameTime - 1;
                        myRoom.underAttack = true; myRoom.underAttackTick = gameTime - 1
            ...
            else if (cache.enemies[event.data.targetId]) { updateEnemies = true }
        });
        if (updateEnemies) myRoom.updateEnemies()
        // update tombstones
        ...
        if (myRoom.my) {
            ...
            if (eventDestroy) {
                console.log(colorful(`${roomLink(room.name)} - EVENT_OBJECT_DESTROYED - ${eventDestroy.data.type}\n${JSON.stringify(eventDestroy)}`, 'red'))
            ...
            if (eventNuke) {
                console.log(colorful(`${roomLink(room.name)} - EVENT_ATTACK_TYPE_NUKE\n${JSON.stringify(eventNuke)}`, 'red'))
                myRoom.updateStructures(); myRoom.updateQueuesAutoBuild()
            }
        }
        else {
