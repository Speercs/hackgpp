'use strict'
//2023.10.11 priority for manage if manager == 0
//2023.09.27 flag for pc
//2023.09.19 operat tower only if myRoom underAttack
...
powerCreepsLogick.main = function () {
    ...
    powerCreeps.forEach(pcName => {
        ...
        if (pc.memory.pos) { pc.travelTo(new RoomPosition(pc.memory.pos.x, pc.memory.pos.y, pc.memory.pos.roomName)); return }
...
PowerCreep.prototype.disruptSpawn = function () {
    const power = PWR_DISRUPT_SPAWN
    if (this.store.getUsedCapacity(RESOURCE_OPS) >= POWER_INFO[power].ops) {
        const targetSpawn = _obj(this.memory.targetSpawn)
    }
    else {
        const myRoom = myRooms[this.memory.room]
...
PowerCreep.prototype.operateTower = function () {
    const power = PWR_OPERATE_TOWER, myRoom = this.memory.room
    if (!myRoom || !myRoom.underAttack || this.store.getUsedCapacity(RESOURCE_OPS) < POWER_INFO[power].ops) { this.spliceTask() }
    else {
        const myRoom = myRooms[this.memory.room]
...
PowerCreep.prototype.getStatus = function () {
    ...
    const priority = {
        [PWR_OPERATE_EXTENSION]: myRoom.tasks.manage.forced_requiredWorkers == 0 ? 1 : 5,
        [PWR_OPERATE_SPAWN]: 2,
        [PWR_OPERATE_TOWER]: 3,
        [PWR_FORTIFY]: 4,
        [PWR_OPERATE_FACTORY]:  myRoom.tasks.manage.forced_requiredWorkers == 0 ? 3 : 5,
        [PWR_REGEN_MINERAL]: 6,
        [PWR_REGEN_SOURCE]: 7,
        [PWR_OPERATE_LAB]: 8,
        manage: myRoom.tasks.manage.forced_requiredWorkers == 0 ? 4 : 9
    }
    const taskList = cache.powerCreepsTasks[this.name]
...
powerCreepsLogick.run = function (pc) {
    ...
    if (pc.ticksToLive < 1000 && pc.room.name == pc.memory.room) { pc._renew(); return }
    if (!myRoom) { return }
    if (!myRoom.my) { return }
