'use strict'
//2023.10.07 flag name for build queue
//2023.09.28 repeat for nukes
        ...
        switch (flag.color) {
            case COLOR_WHITE:
                ...
                if (!squad) { squadTools.createSquad(flag.secondaryColor, flag.name, flag.pos); squads[flag.secondaryColor].status = 'moveAttack'; squads[flag.secondaryColor].pos = flag.pos }
                    ...
                    else if (flag.name == 'p') { squad.path = squad.path || []; squad.path.push(flag.pos); squad.pos = flag.pos; squad.backup() }
            ...
            case COLOR_YELLOW:
                ...
                function pushToBuildQueue(structureType, done = false) {
                    newBuild = { structureType: structureType, pos: flag.pos, done: done }
                    queue.push(newBuild); myRoom.room.memory.structurePositions.push({ type: structureType, pos: flag.pos })
                    console.log(colorful(`adding structure: ${structureType} to ${flag.pos}`, 'orange'))
                }
                if (flag.name.substring(0, 4) !== 'Flag') {
                    if (STRUCTURES.includes(flag.name)) { pushToBuildQueue(flag.name) }
                }
                else {
                    switch (flag.secondaryColor) {
                        case COLOR_RED: pushToBuildQueue(STRUCTURE_STORAGE, flag.name == 'true' ? true : false); break
                        case COLOR_PURPLE: pushToBuildQueue(STRUCTURE_TOWER, flag.name == 'true' ? true : false); break
                        case COLOR_BLUE: pushToBuildQueue(STRUCTURE_LINK, flag.name == 'true' ? true : false); break
                        case COLOR_CYAN: pushToBuildQueue(STRUCTURE_TERMINAL, flag.name == 'true' ? true : false); break
                        case COLOR_GREEN: pushToBuildQueue(STRUCTURE_SPAWN, flag.name == 'true' ? true : false); break
                        case COLOR_YELLOW: pushToBuildQueue(STRUCTURE_EXTENSION, flag.name == 'true' ? true : false); break
                        case COLOR_ORANGE: pushToBuildQueue(STRUCTURE_LAB, flag.name == 'true' ? true : false); break
                        case COLOR_GREY: pushToBuildQueue(STRUCTURE_ROAD, flag.name == 'true' ? true : false); break
                        case COLOR_WHITE:
                ...
                break
            case COLOR_ORANGE:
                const PC = _.find(Game.powerCreeps, pc => pc.memory.flag == flag.secondaryColor)
                if (PC) PC.memory.pos = flag.pos
                flag.remove()
                break
            case COLOR_BROWN:
                ...
                if (flag.secondaryColor == COLOR_BROWN) {
                    ...
                    let roomName = '', repeat = false
                    if (flag.name !== 'Flag1') {
                        if (flag.name == 'repeat') repeat = true
                        else roomName = flag.name
                    }
                    const newTask = { pos: flag.pos, roomName, repeat }
