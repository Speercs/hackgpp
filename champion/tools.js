...
tools.updateAvoidRooms = function () {
    ...
    Object.keys(Memory.rooms).forEach(roomName => {
        if (Memory.rooms[roomName].avoid && !Memory.rooms[roomName].ignoreAvoid) { Memory.avoidRooms[roomName] = true }
    })
}
        ...
        try {
            let route = Game.map.findRoute(room1.name, room2.name, {
                routeCallback(roomName) {
                    if (Memory.avoidRooms[roomName]) { return Infinity; }  // avoid this room
                    return 1;
                }
            })
...
tools.launchNukeByTasks = function () {
    ...
    Memory.nukeTaskList.forEach(task => {
        ...
        if (roomName) {
            ...
            // console.log(`readyNukers:${readyNukers}`)
