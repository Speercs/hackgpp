'use strict'
//2023.11.15 clearing storage link if need transfer from source
//2023.11.09 updateLabReaction - exclude 
    ...
    function transferToStorageLink(myRoom) {
        ...
        for (const link of links) {
            ...
            if (type == 'source') {
                ...
                else if (!storageLink.store.getFreeCapacity(RESOURCE_ENERGY)) { myRoom.registerSheduledTask('clearing storage link') }
            }
        }
    }
...
MyRoom.prototype.updateLabReaction = function (exclude = false) { //choose reaction
    const cpu = settings.writePerformanceLog ? Game.cpu.getUsed() : 0
    ...
    let choosenResource
    if (exclude) { cache.resourcesSortedToChooseByLabs.cut(exclude) }
...
MyRoom.prototype.boostCreeps = function () {
    ...
    for (const resourceType of resources) {
        const labsObj = _.filter(this.room.memory.labs, lab => lab.resourceType == resourceType); if (!labsObj) continue
        ...
        idToObj(labsObj.map(lab => lab.id)).forEach(lab => {
