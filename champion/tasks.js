'use strict'
//2023.10.14
//2023.10.03 check queues by memory.task
//2023.09.29 timeOut for spawn
//2023.09.23 //edge of the room for dismantle
//2023.09.11 //get body from roles
global.tasks = {}
global.taskTools = {}
    ...
    constructor(type, pos, number = false, specifications = false) {
        ...
        if (!number) {
            switch (type) {
                ...
                case 'claim': this.roles = { observer: { role: 'observer', amount: 0 }, claim: { role: 'claim', amount: 0 } }
                    spawnRoom = tools.findNearestRoom({ roomName: pos.roomName, minimumLevel: 3 }); break;
                case 'build base': this.roles = { 'build base': { role: 'build base', amount: 0 } }
                ...
                case 'build base to ally': this.roles = { 'build base': { role: 'build base', amount: 1 } }
                    spawnRoom = tools.findNearestRoom({ roomName: pos.roomName, excludeRoom: true, minimumLevel: 8 }); break
                case 'transfer': this.roles = { 'transfer': { role: 'transfer', amount: 0 } }
                ...
                case 'dismantleRoom': this.roles = { 'dismantleRoom': { role: 'dismantleRoom', amount: 0 } }
                    spawnRoom = tools.findNearestRoom({ roomName: pos.roomName, excludeRoom: false, minimumLevel: 7 }); break
            ...
            if (spawnRoom) {
                ...
                // const avoidRooms = Object.keys(Memory.avoidRooms).filter(r => tools.getDistanceForRooms(r, spawnRoom) <= 10)
                const route = Game.map.findRoute(this.roomName, spawnRoom, {
                    routeCallback(roomName) {
                        if (Memory.avoidRooms && Memory.avoidRooms[roomName]) { return Infinity; }  // avoid this room
                        return 1;
                    }
                })
...
Task.prototype.checkSpawn = function () {
    ...
    if (this.timeOut && this.timeOut > Game.time) { return } else { delete this.timeOut }
    ...
    if (this.type == 'claim') {
        Object.keys(this.roles).forEach(role => {
            const inQueues = this.spawnRoom.queues.spawn.filter(s => s.role == role && s.memory.task == this.number).length
    ...
    else if (this.type == 'build base' && (this.forceSpawn || (myRooms[this.pos.roomName] && !myRooms[this.pos.roomName].my))) { return }
    else if (this.type == 'attackGroup') {
        if (!_.filter(Game.creeps, creep => creep.memory.task == this.number).length) { this.spawnStatus = true; delete this.status }
    ...
    Object.keys(this.roles).forEach(role => {
        ...
        const different = this.roles[role].amount - ((countByRole[role]) ? countByRole[role].amount : 0) - inQueues
        if (different > 0) { for (let i = 0; i < different; i++) { this.spawn({ role: role }) } }
        else if (this.spawnByTimer && !this.timeOut) { this.timeOut = gameTime + this.spawnByTimer }
    })
...
Task.prototype.spawn = function (specifications) {
    ...
    let body
    if (role && this.roles[role].body && this.roles[role].body.length) { body = this.roles[role].body }
    else {
        if (this.type == 'claim') {
...
taskTools.checkTasksHarvestDeposits = function () {
    ...
    if (settings.tasks.harvestDeposit) {
    ...
    if (settings.tasks.harvestPower) {
...
Task.prototype.action = function () {
    ...
    if (task.type == 'claim') {
        ...
        claimCreeps.forEach(claim => {
            if (task.upgradeBlocked && claim.ticksToLive < task.upgradeBlocked - gameTime) { claim.suicide(); return }
            ...
            if (claim.pos.roomName == pos.roomName) {
                if (claim.room.controller) {
                    if (claim.pos.getRangeTo(claim.room.controller) > 1) { claim.travelTo(claim.room.controller) }
                        ...
                        if (claim.room.controller.reservation && claim.room.controller.reservation.username !== settings.username) {
                            claim.attackController(claim.room.controller)
                        }
                    }
                }
...
Task.prototype._transfer = function () {
    ...
    transferList.forEach(creep => {
        ...
        if (!creep.store.getFreeCapacity()) { creep.memory.status = 'transfer' }
            ...
            if (this.drop) { if (creep.room.name == pos.roomName && creep.ticksToLive < task.distance * 2) { creep.suicide(); return } }
...
Task.prototype.buildBase = function () {
    ...
    else if (this.manage && myRoom.room.energyAvailable < myRoom.room.energyCapacityAvailable) { task.status = 'manage' }
    ...
    task.creeps.forEach(creep => {
        ...
        if (creep.memory.status == 'manage') { creep.fillEnergyToStructures() }
        else if (creep.memory.status == 'withdraw') {
            if (creep.memory.ruin) {
                const ruin = _obj(creep.memory.ruin)
                if (ruin && ruin.store['energy']) { creep._withdraw(ruin); return }
                else { delete creep.memory.ruin }
            }
            if (creep.memory.dismantle) {
                ...
                if (task.ruins) {
                    const ruin = creep.pos.findClosestByPath(FIND_RUINS, { filter: s => s.store && s.store.getUsedCapacity('energy') })
                    if (ruin) { creep.memory.ruin = ruin.id; creep._withdraw(ruin); return }
                }
                ...
                const controllerContainers = tools.getContainersList(task.pos.roomName, 'controller').filter(c => c.store[RESOURCE_ENERGY])

                if (controllerContainers.length) {
                    creep._withdraw(controllerContainers.sort((a, b) => b.store.getUsedCapacity() - a.store.getUsedCapacity())[0]); return
                }
    ...
    function build(creep, task) {
        ...
        if (creep.memory.debug) console.log(buildObject)
        if (!buildObject) { creep.resetStatus() }
        else if (buildObject && myRooms[task.pos.roomName].constructionSites.includes(task.buildObject)) { if (creep.build(buildObject) == ERR_NOT_IN_RANGE) { creep.travelTo(buildObject, { range: 3 }) } }
        else { creep.memory.targetId = buildObject.id; creep._repair(); }
    ...
    function taskUpdateBuildObject(task) {
        ...
        if (myRoom.constructionSites.length) {
            ...
            const targets = (_.filter(cache.enemies, e => !e.hiden && e.pos.roomName == task.pos.roomName).length) ? [] : task.creeps.map(creep => creep.memory.targetId)
            let ramparts = idToObj(myRooms[task.pos.roomName].structures.ramparts.filter(id => !targets.includes(id)))
            const walls = idToObj(myRooms[task.pos.roomName].structures.walls.filter(id => !targets.includes(id))).filter(x => x.hits)
            ramparts = ramparts.concat(walls).sort((a, b) => a.hits - b.hits)
            if (ramparts.length) { task.buildObject = ramparts[0] }
            return false
        }
    }
    ...
    function updateSources(task) {
        ...
        let sources = idToObj(_.map(myRoom.tasks.harvestSource, source => source.id))
        if (task.spawnRoom.level > 4) {
            sources = sources.filter(s => !usedSources.includes(s.id))
        }
...
Task.prototype.dismantleRoom = function () {
    ...
    let pos2; if (task.pos2) pos2 = new RoomPosition(task.pos2.x, task.pos2.y, task.pos2.roomName)
    const taskDismantleObject = _obj(task.dismantleId)
    ...
    dismantleCreeps.forEach(creep => {
        ...
        creep.notifyWhenAttacked(false)
        if (creep.hits < creep.hitsMax || (creep.room.name == task.pos.roomName && taskDismantleObject && !creep.pos.inRangeTo(taskDismantleObject, 1))) { creep.heal(creep) }
        if (pos2 && creep.hits < creep.hitsMax) { creep.travelTo(pos2); return }
        if (pos2 && taskDismantleObject
            && (taskDismantleObject.pos.findInRange(FIND_HOSTILE_CREEPS, 2, { filter: s => cache.enemies[s.id] && cache.enemies[s.id].parameters && cache.enemies[s.id].parameters.attack }).length
                || taskDismantleObject.pos.findInRange(FIND_HOSTILE_CREEPS, 3, { filter: s => cache.enemies[s.id] && cache.enemies[s.id].parameters && cache.enemies[s.id].parameters.rangedAttack > 500 }).length)
            || (creep.pos.roomName == pos.roomName &&
                creep.pos.findInRange(FIND_HOSTILE_CREEPS, 2, { filter: s => cache.enemies[s.id] && cache.enemies[s.id].parameters && cache.enemies[s.id].parameters.attack }).length)) {

            creep.travelTo(pos2); return
        }
        if (creep.room.name == task.pos.roomName && !myRooms[task.pos.roomName].my) {
            if (creep.memory.isStuck && creep.pos.edgeOfTheRoom()) {
                if (creep.pos.x == 49) {
                    if (chekPosForMove({ x: creep.pos.x - 1, y: creep.pos.y, roomName: creep.pos.roomName }, creep)) { creep.moveTo(creep.pos.x - 1, creep.pos.y) }
                    else if (chekPosForMove({ x: creep.pos.x - 1, y: creep.pos.y - 1, roomName: creep.pos.roomName }, creep)) { creep.moveTo(creep.pos.x - 1, creep.pos.y - 1) }
                    else if (chekPosForMove({ x: creep.pos.x - 1, y: creep.pos.y + 1, roomName: creep.pos.roomName }, creep)) { creep.moveTo(creep.pos.x - 1, creep.pos.y + 1) }
                }
                    ...
                    if (chekPosForMove({ x: creep.pos.x, y: creep.pos.y - 1, roomName: creep.pos.roomName }, creep)) { creep.moveTo(creep.pos.x, creep.pos.y - 1) }
                    else if (chekPosForMove({ x: creep.pos.x - 1, y: creep.pos.y - 1, roomName: creep.pos.roomName }, creep)) { creep.moveTo(creep.pos.x - 1, creep.pos.y - 1) }
                    else if (chekPosForMove({ x: creep.pos.x + 1, y: creep.pos.y - 1, roomName: creep.pos.roomName }, creep)) { creep.moveTo(creep.pos.x + 1, creep.pos.y - 1) }
                }
        ...
        if (creep.pos.roomName !== pos.roomName) { creep.travelTo(pos) }
            ...
            if (creep.hits == creep.hitsMax) {
                if (structure) {
                    ...
                    if (creep.pos.findInRange(FIND_STRUCTURES, 2, { filter: s => s.structureType == 'constructedWall' || s.structureType == 'rampart' }).length) { creep.travelTo(pos); if (!attackHostile()) { creep.rangedAttack3() } }
                    else if (range > 3) { creep.travelTo(structure, { range: 3 }); if (!attackHostile()) { creep.rangedAttack3() }; return }
...
Task.prototype.attackGroup = function () {
    ...
    let firstLine = rc, secondLine = hc, width = secondLine
...
Task.prototype.attackGroup_backup = function () {
    const task = this, pos = new RoomPosition(this.pos.x, this.pos.y, this.pos.roomName), room = Game.rooms[pos.roomName]
    task.direction = task.direction || 1
    const creeps = [...this.creeps], creepsId = creeps.map(c => c.id), less70List = creeps.filter(c => c.hits < c.hitsMax * 0.7).sort((a, b) => a.hits - b.hits)
    let less70; if (less70List.length) less70 = less70List[0]
    ...
    task.creeps.filter(creep => creep.memory.moveByPathList_done).forEach(creep => {
        ...
        if (creep.memory.role == 'partizan_dismantle') {
            ...
            function chooseObjectToMove() {
                ...
                // if (obj1) { return obj1 }
                const obj3 = creep.pos.findClosestByPath(FIND_HOSTILE_CREEPS, { ignoreCreeps: true })
                if (obj3) { return obj3 }
