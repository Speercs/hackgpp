'use strict'
//2023.11.15 ignoreRepairsForLinks and upgraders = 0
//2023.11.09 updateLabReaction exclude current reaction
//2023.11.01 transfer res when prepare lab
//2023.10.08 settings.fillEnergy
//2023.09.20 fix fill controller link when storage < 10 000
//2023.09.16 fix for object2
//2023.09.15 fix about containers
//2023.08.28 tombstones with enemies 
//2023.08.19 containers base
...
Creep.prototype.manage_transfer = PowerCreep.prototype.manage_transfer = function () {
    ...
    let containersBase = []
    if (myRoom.level <= 4) {
        const containersBaseId = myRoom.structures.containers.filter(c => !c.type).map(c => c.id)
        if (containersBaseId.length) containersBase = idToObj(containersBaseId)
        if (debug) debugText += `containersBase:${containersBase}\n`
        if (containersBase.length && containersBase.map(c => c.store.getUsedCapacity(resourceType)).reduce((a, b) => a + b, 0) >= amount) {
            const containerForTransfer = containersBase.sort((a, b) => a.store.getUsedCapacity(resourceType) - b.store.getUsedCapacity(resourceType))[0]
            this._transfer(containerForTransfer, resourceType, amount < containerForTransfer.store.getUsedCapacity(resourceType) ? amount : containerForTransfer.store.getUsedCapacity(resourceType))
        }
    }
}
const manage_withdraw = function () {
    ...
    if (storage && terminal) {
        ...
        if (myRoom.level <= 4) {
            ...
            if (containersBaseId.length) containersBase = idToObj(containersBaseId)
            if (debug) debugText += `containersBase:${containersBase}\n`
            if (containersBase && containersBase.length) {
                if (containersBase.map(c => c.store.getUsedCapacity(resourceType)).reduce((a, b) => a + b, 0) >= amount) {
                    if (debug) debugText += `_withdraw\n`
                    const containerToWithdraw = containersBase.sort((a, b) => b.store.getUsedCapacity(resourceType) - a.store.getUsedCapacity(resourceType))[0]
                    this._withdraw(containerToWithdraw, resourceType, amount - this.store[resourceType] < containerToWithdraw.store[resourceType] ? amount - this.store[resourceType] : containerToWithdraw.store[resourceType])
                    if (debug) { console.log(debugText) }; return
                }
            }
        }
...
const manage_setResourceToWithdraw = function () {
    ...
    let done = true
    const resources = Object.keys(this.memory.resources)
    if (!resources.length) { console.log(this, 'memory.resources.length == 0'); this.resetStatus(); return }
...
const manage_gettingTasks = function () {
    ...
    let lastObj = myRoom.storage
    if (!lastObj && containersBase.length) lastObj = containersBase[0]
...
const manage = function () {
    ...
    if (task.type == 'prepare lab to boost') {
        ...
        if (task.status == 'fill energy') {
            ...
            if (exEnergyRes.length) {
                ...
                if (resInLab !== task.resourceType) {
                    ...
                    else { this._transfer(terminal.store.getFreeCapacity() > storage.store.getFreeCapacity() ? terminal : storage, Object.keys(this.store)[0]); return }
                }
                else { if (lab.store.getUsedCapacity(task.resourceType) == task.amount) { delete this.memory.object1; myRoom.manage_deleteMiniTask(task); task.boost.ready = true; this.resetStatus(); return } }
            }
            // get free store of this if resource type error
            if (this.store.getUsedCapacity()) { if (Object.keys(this.store)[0] !== task.resourceType) { this._transfer(terminal.store.getFreeCapacity() > storage.store.getFreeCapacity() ? terminal : storage, Object.keys(this.store)[0]); return } }
        ...
        if (!object2) { delete this.memory.object1; myRoom.manage_deleteMiniTask(task); return }
        ...
        if (myRoom.enemies.length && myRoom.room.memory.base && !myRoom.room.memory.base.find(p => p.x == object1.pos.x && p.y == object1.pos.y)) { myRoom.manage_deleteMiniTask(task); return }
...
MyRoom.prototype.manage = function () {
    ...
    creeps.forEach(creep => {
        ...
        if (creep.spawning || creep.rescueFromNuke()) { return }
...
MyRoom.prototype.updateManageMicroTasks_storage_terminal = function (resource = false) {
    ...
    if (unclaimThisRoom) {
        if (this.settings.fillEnergy) {
            let priority = 7
            const resourcesInStorage = Object.keys(storage.store).filter(key => key !== 'energy')
            if (resourcesInStorage.length) {
                resourcesInStorage.forEach(resourceType => {
                    this.registerManageTask({ object1: storage, object2: terminal, resourceType: resourceType, amount: storage.store[resourceType], type: 'transfer', priority: priority })
                })
            }
            if (Object.keys(terminal.store).filter(key => key !== 'energy').length === 0 && resourcesInStorage.length === 0) {                
                if (storage.store.getFreeCapacity('energy')) {
                    this.registerManageTask({ object1: terminal, object2: storage, resourceType: 'energy', amount: storage.store.getFreeCapacity('energy'), type: 'transfer', priority: priority })
                }
            }
        }
        else if (gameTime !== scriptLoadTime && Object.keys(terminal.store).filter(key => key !== 'energy').length === 0 && Object.keys(storage.store).filter(key => key !== 'energy').length === 0) {
            let priority = 7
...
MyRoom.prototype.updateManageMicroTasks_labs = function () {
    ...
    if (this.unclaim && !this.settings.fillEnergy && this.terminal) {
        ...
        if (labsOut.length == 2) {
            ...
            if (!reverse) {
                ...
                if (res2 && res2 !== resource2) { this.registerManageTask({ object1: lab2, object2: terminal, resourceType: res2, amount: lab2.store[res2], type: 'transfer', priority: priority }) }
                    ...
                    if (!amount) { if (lab2.store[resource2]) { this.updateLabReaction(this.room.memory.labs.reaction) }; return }
...
MyRoom.prototype.updateManageMicroTasks_towers = function () {
    ...
    if (towers.length) {
        for (const structure of towers) {
            this.registerManageTask({ object1: structure, amount: structure.store.getFreeCapacity(RESOURCE_ENERGY), type: 'fill structure', priority: 5 })
        }
    }
}
MyRoom.prototype.updateManageMicroTasks_containers = function () {
    ...
    if (this.settings.debug.manage) { console.log(`containers_controller.length:${containers_controller.length}, containerBase:${containerBase}`) }
    if (!storage && containers_controller.length && containerBase && containers_base.map(c => c.store['energy']).reduce((a, b) => a + b, 0) > 1000) {
        ...
        if (containerController.store.getFreeCapacity('energy')) {
            this.registerManageTask({ object1: containerBase, object2: containerController, resourceType: 'energy', amount: containerBase.store['energy'] - 1000, type: 'transfer', priority: 6 })
        }
    }
}
                    ...
                    if (this.status !== 'support' || settings.rotateRooms) {
                        if (!lvl) {
                            ...
                            lvl1.forEach(res => {
                                ...
                                if (factory.store[res] > 4000) { getRes(res, 1250, storage, terminal, factory, this) }
                            })
...
MyRoom.prototype.updateManageMicroTasks = function (specifications = false) {
    ...
    if (unclaimThisRoom && !this.settings.fillEnergy) { this.tasks.manage.tasks.filter(t => t.type == 'fill structure').forEach(t => this.tasks.manage.tasks.cut(t)) }

    if (!unclaimThisRoom || this.settings.fillEnergy) {
        if (specifications) {
    ...
    if (Object.keys(this.structures.links).length) {
        ...
        const amountRoleRepair = this.settings.ignoreRepairsForLinks ? 0 : _.filter(this.creeps, creep => creep.memory.role == 'repair').length
        const amountRoleUpgrade = 0 //_.filter(this.creeps, creep => creep.memory.role == 'upgrade').length && this.level < 8
        ...
        if (storageLinkMemory && this.storage) {
            ...
            if ((this.storage.store['energy'] > 10000 && amountRoleRepair && !this.settings.ignoreRepairsForLinks) || (this.storage.store['energy'] > 10000 && controllerLinkId && amountRoleUpgrade)) {
    ...
    if (this.structures.nuker && (!this.lastUpdateMicroTasks_nuker || gameTime - this.lastUpdateMicroTasks_nuker >= 1000)) {
        ...
        if (nuker && this.terminal) {
            ...
            if (needEnergy) {
                if (this.storage && this.storage.store.getUsedCapacity(RESOURCE_ENERGY) > 20000) {
