'use strict'
//2023.11.15 cache.terminals.ordersList = [] by * 10
//2023.11.01 change minS & maxS to minimumStock & maximumStock
...
terminal_logic.internalExchange = function (resource = false) {
    ...
    if (settings.frequency.internalExchange.frequency >= 100 && Game.time % (settings.frequency.internalExchange.frequency * 10) == 0) { cache.terminals.ordersList = [] }//; console.log('clearing internal exchange order list...') }
    ...
    Memory.roomsToUnclaim.forEach(name => {
        ...
        if (myRoom.terminal) {
            const terminal = myRoom.terminal
            if (myRoom.settings.fillEnergy && terminal.store.getFreeCapacity('energy')) {
            ...
            if (terminals.length) {
                ...
                if (targetRoom) {
                    ...
                    if (!myRoom.settings.fillEnergy && resources.length === 0) {
...
MyRoom.prototype.buyOrSellResources = function () {
    ...
    const myOrders = cache.market.orders = cache.market.orders || Game.market.orders
    resources.forEach(resourceType => {
        const min = minimumStock(resourceType), max = maximumStock(resourceType)
        if (this.resources[resourceType] < min) {
            ...
            cache.roomsOfAlly = cache.roomsOfAlly || []
            if (cache.terminals.busyAtCurrentTick.includes(this.name)) { return }
...
terminal_logic.buyResources = function (resourcesToBuy = ['U', 'O', 'H', 'K', 'L', 'Z', 'X'], force = false) {
    ...
    resourcesToBuy.forEach(resourceType => {
        ...
        const min = minimumStock(resourceType)
