'use strict'
//2023.09.30 update lab reaction every 1000 ticks
//2023.08.17 filter cache.myRoom by room
global.survey = {}
...
survey.surveyRooms = function () {
    ...
    cache.myRooms.filter(mr => mr.room).sort((a, b) => a.room.controller.ticksToDowngrade - b.room.controller.ticksToDowngrade).forEach(myRoom => {
        const myRoomLog = myRoom ? myRoom.name : 'room name unknown'
        myRoom.room = Game.rooms[myRoom.name]; myRoom.cache = {}
        if (!myRoom.room || !myRoom.room.controller.my) { return }
        ...
        if (!status && ((!myRoom.room.memory.labs.reaction && each100) || each1000)) { try { myRoom.updateLabReaction() } catch (error) { console.log(`${myRoomLog}`, 'updateLabReaction', error + `\n${error.stack}`) } }
    })
