...
Squad.prototype.checkSpawn = function () {
    ...
    if (Game.rooms[this.pos.roomName] && Game.rooms[this.pos.roomName].controller && Game.rooms[this.pos.roomName].controller.safeMode) { this.timeOut = Game.time + Game.rooms[this.pos.roomName].controller.safeMode }
...
Squad.prototype.moveTo = function () {
    ...
    creeps.forEach(creep => {
        if (pos2 && (creep.hits < creep.hitsMax * 0.99 || creep.pos.findInRange(FIND_HOSTILE_CREEPS, 2, { filter: s => cache.enemies[s.id].parameters.attack || cache.enemies[s.id].parameters.rangedAttack > 500 }).length)) {
            creep.travelTo(pos2)
            creep.say('back')
        }
        else {
...
Squad.prototype.attack = function () {
    ...
    const creeps = this.creeps
    const pos = new RoomPosition(this.pos.x, this.pos.y, this.pos.roomName)
    let pos2; if (this.pos2) pos2 = new RoomPosition(this.pos2.x, this.pos2.y, this.pos2.roomName)
...
Squad.prototype.rangedAttackAndHeal = function () {
    ...
    const heal = this.creeps.find(c => c.memory.role == 'heal')
    const pos = new RoomPosition(this.pos.x, this.pos.y, this.pos.roomName)
    const criticalDamage = this.criticalDamage || 300
    const creepBoost = []
    ...
    if (rangedAttack && heal && !creepBoost.length) {
        if (rangedAttack.room.name == pos.roomName) {
            ...
            if (rangedAttack.hitsMax - rangedAttack.hits + rangedAttack.hitsMax - heal.hits > criticalDamage || rangedAttack.pos.findInRange(FIND_HOSTILE_CREEPS, 2, { filter: s => cache.enemies[s.id].parameters.attack || cache.enemies[s.id].parameters.rangedAttack }).length || heal.pos.findInRange(FIND_HOSTILE_CREEPS, 2, { filter: s => cache.enemies[s.id].parameters.attack || cache.enemies[s.id].parameters.rangedAttack }).length) {
        ...
        if (heal.hits >= rangedAttack.hits) { if (heal.heal(rangedAttack) !== OK) { if (heal.rangedHeal(rangedAttack) !== OK) { heal.heal(heal) } } }
        else { heal.heal(heal) }
    ...
    function healMoveTo(rangedAttack, heal) {
        if (rangedAttack.memory.isStuck
            ...
            if (rangedAttack.pos.x == 1 || rangedAttack.pos.x == 48) {
                ...
                else if (chekPosForMove({ x: rangedAttack.pos.x, y: rangedAttack.pos.y + 1, roomName: rangedAttack.pos.roomName }, heal)) { heal.moveTo(rangedAttack.pos.x, rangedAttack.pos.y + 1) }
            }
