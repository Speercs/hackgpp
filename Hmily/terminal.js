...
terminal_logic.sellCommidiates = function (resourceType, lowPrice) {
    const cpu = settings.writePerformanceLog ? Game.cpu.getUsed() : 0
    const terminals = []
    ...
    let orders
    orders = Game.market.getAllOrders(order => order.amount > 0 && order.price >= lowPrice && order.type == ORDER_BUY && order.resourceType == resourceType)
...
MyRoom.prototype.buyOrSellResources = function () {
    ...
    resources.forEach(resourceType => {
        ...
        if (this.resources[resourceType] < min) {
        ...
        else if (settings.terminals.sellResources && this.resources[resourceType] > max * 0.75 && !_.find(myOrders, mo => mo.resourceType == resourceType && mo.type == ORDER_BUY)) {
            if (cache.terminals.busyAtCurrentTick.includes(this.name)) { return }
            const orders = Game.market.getAllOrders(order => order.amount > 0 && order.type == ORDER_BUY && order.resourceType == resourceType).sort((a, b) => b.price - a.price)
            ...
            for (const order of orders) {
                ...
                if (result == OK) {
                    ...
                    console.log(colorful(`${roomLink(this.name)}: sell to ${roomLink(order.roomName)} - ${order.resourceType} amount: ${amount} for price: ${order.price}, credits: ${amount * order.price}, credits: ${tools.numberWithSpaces(Game.market.credits)}`, 'green'))
                    break
                }
            }
        }
