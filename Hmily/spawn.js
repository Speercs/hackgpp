...
spawnLogick.getBody = function (role, energy, specifications = false) {
    ...
    switch (role) {
        case 'partizan_rangedAttack':
            ...
            return body
        case 'attackRoom':
            body.pushN(TOUGH, 7); body.pushN(RANGED_ATTACK, 5); body.pushN(TOUGH, 8); body.pushN(RANGED_ATTACK, 5); body.pushN(MOVE, 9); body.pushN(HEAL, 15); body.push(MOVE);
            // body.pushN(TOUGH, 2); body.pushN(RANGED_ATTACK, 8); body.pushN(TOUGH, 4); body.pushN(MOVE, 9); body.pushN(RANGED_ATTACK, 16); body.pushN(HEAL, 10); body.push(MOVE);
            // move = moveByBoost()
        ...
        case 'harvest mineral': //switch (true) {
            if (specifications.container) {
                ...
                if (workMoveCarry) { work = workMoveCarry.work; carry = workMoveCarry.carry; move = workMoveCarry.move }
            }
            break
        case 'repair':
...
function getCreepName(role = false) {
    const pre = ['a', "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "w", "z"]
    return (tools.pickRandom(pre) + Math.floor(Math.random() * 1000000))
}
...
global.createBody = function () {
    Memory.createBody = Memory.createBody || {}
    Memory.createBody.body = Memory.createBody.body || []
    Memory.createBody.energy = Memory.createBody.energy || 12900
