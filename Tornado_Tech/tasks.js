...
Task.prototype.buildBase = function () {
    ...
    function updateSources(task) {
        ...
        if (task.spawnRoom.level > 4) {
            sources = sources.filter(id => !usedSources.includes(id))
        }
...
Task.prototype.dismantleRoom = function () {
    ...
    dismantleCreeps.forEach(creep => {
        ...
        creep.notifyWhenAttacked(false)
        if (creep.hits < creep.hitsMax && task.pos2) { creep.travelTo(new RoomPosition(task.pos2.x, task.pos2.y, task.pos2.roomName)); return }
