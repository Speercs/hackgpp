...
global.showLab = function () {
    ...
    labRooms.forEach(myRoom => {
        msg += `${roomLink(myRoom.name)}\tupdateLabReaction: ${myRoom.updateLabReactionTick}\t${JSON.stringify(myRoom.room.memory.labs, 0, 3)}\n`
...
global.showPow = function () {
    let msg = `settings.tasks.harvestPower: ${settings.tasks.harvestPower}\n`
...
global.showRes = function (resourceType = false, showIfZero = true) {
    if (resourceType !== false) {
        if (resourceType == 4) {
            let msg = '', sum = 0
            resourcesTier4.forEach(res => { msg += res + ': ' + tools.numberWithSpaces(cache.resources[res]) + '\t' + (descriptions[res] ? descriptions[res] : '') + '\n'; sum += cache.resources[res] })
            return 'sum: ' + tools.numberWithSpaces(sum) + '\n' + msg
        }
        else if (resourceType == 3) {
            let msg = '', sum = 0
            resourcesTier3.forEach(res => { msg += res + ': ' + tools.numberWithSpaces(cache.resources[res]) + '\t' + (descriptions[res] ? descriptions[res] : '') + '\n'; sum += cache.resources[res] })
            return 'sum: ' + tools.numberWithSpaces(sum) + '\n' + msg
        }
        else if (resourceType == 2) {
            let msg = '', sum = 0
            resourcesTier2.forEach(res => { msg += res + ': ' + tools.numberWithSpaces(cache.resources[res]) + '\t' + (descriptions[res] ? descriptions[res] : '') + '\n'; sum += cache.resources[res] })
            return 'sum: ' + tools.numberWithSpaces(sum) + '\n' + msg
        }
        else if (resourceType === 0) {
            let msg = '', sum = 0
            resourcesTier1.forEach(res => { msg += res + ': ' + tools.numberWithSpaces(cache.resources[res]) + '\t' + (descriptions[res] ? descriptions[res] : '') + ` (${cache.resources[tools.getResToCompressOrDecompress(res)]})` + '\n'; sum += cache.resources[res] })
            return 'sum: ' + tools.numberWithSpaces(sum) + '\n' + msg
        }
        else {
        ...
        res.forEach(resourceType => {
            if (showIfZero) { msg += `${tools.numberWithSpaces(cache.resources[resourceType])}\t${resourceType}\t${descriptions[resourceType] ? descriptions[resourceType] : ''} \n` }
            else if (cache.resources[resourceType] && cache.resources[resourceType] > 0) { msg += `${resourceType}\t${cache.resources[resourceType]}\t${descriptions[resourceType] ? descriptions[resourceType] : ''} \n` }
        })
        return msg
    }
}
