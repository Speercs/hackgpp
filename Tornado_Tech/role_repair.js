...
MyRoom.prototype.repair = function () {
    ...
    const targets = (_.filter(cache.enemies, e => !e.hiden && e.pos.roomName == this.name).length || this.nukes.length) ? [] : creeps.map(creep => creep.memory.targetId)


    if (this.storage) {
        if (this.resources[RESOURCE_ENERGY] > 10000 && this.constructionSites.length) { chooseBuildObject(this, task); task.status = 'build' }
        ...
        if (this.terminal.store[RESOURCE_ENERGY] > 1000 && this.constructionSites.length) { chooseBuildObject(this, task); task.status = 'build' }
        ...
        if (this.constructionSites.length) { chooseBuildObject(this, task) }
        else task.status = 'repair'
    }

    function chooseBuildObject(myRoom, task) {
        const buildObject = _obj(task.buildObjectId)
        if (!buildObject) {
            delete task.buildObject
            if (task.buildObjectId) { if (myRoom.constructionSites.includes(task.buildObjectId)) { myRoom.constructionSites.splice(myRoom.constructionSites.indexOf(task.buildObjectId), 1) } }
            if (myRoom.constructionSites.length) {
                const objects = idToObj(myRoom.constructionSites)
                if (objects.length) {
                    task.status = 'build'
                    const buildObject = myRoom.room.controller.pos.findClosestByPath(objects, { ingoreCreeps: true })
            ...
            else if (myRoom.structures.ramparts.length + myRoom.structures.walls.length + myRoom.structures.containers.length + myRoom.structures.roads.length > 0) { task.status = 'repair' }
        }
        ...
        switch (creep.memory.status) {
                ...
                if (creep.memory.withdrawObject) {
                    ...
                    if (target) {
                        if (target.store.getUsedCapacity(RESOURCE_ENERGY) && creep.pos.inRangeTo(target, 1)) {
                            if (target.structureType == 'container' && target.store.getUsedCapacity(RESOURCE_ENERGY) > 1500) {
                                const res = creep.withdraw(target, RESOURCE_ENERGY); if (creep.memory.debug) { console.log(creep, `withdraw res: ${res}`) }
                            }
                            else { const res = creep.withdraw(target, RESOURCE_ENERGY); if (creep.memory.debug) { console.log(creep, `withdraw res: ${res}`) } }
                        }
            ...
            case 'repair':
                ...
                if (!creep.memory.targetId) {
                    ...
                    if (!target && _.filter(cache.enemies, e => !e.hiden && e.pos.roomName == this.name).length < 5) {
...
myRooms_tool.updateTaskRepair = function () {
    ...
    cache.myRooms.filter(mr => !forcedRooms.includes(mr)).forEach(myRoom => {
        ...
        else if (_.filter(myRoom.enemies, e => !e.hiden && e.owner.username !== 'Invader' && (cache.enemies[e.id].parameters.dismantle || cache.enemies[e.id].parameters.attack || cache.enemies[e.id].parameters.rangedAttack)).length) {
