'use strict'
//2023.09.25 manual register boost for build base
//2023.09.22 fix boost if low res
//2023.09.19 heal if enemies near
...
Creep.prototype._repair = function () {
    const myRoom = myRooms[this.memory.roomName] || myRooms[tasks[this.memory.task].pos.roomName]
...
Creep.prototype.registerBoost = function (bodyPart = false) {
    ...
    if (this.memory.role == 'build base') myRoom.registerBoost(this.memory.role, this.name, this.body, bodyPart, { task: this.memory.task })
    else myRoom.registerBoost(this.memory.role, this.name, this.body, bodyPart)
}
Creep.prototype.getBoosted = function () {
    ...
    if (myRoom.room.memory.labsBoostMode) {
        ...
        if (boostQueue.length) {
            ...
            const labsObj = idToObj(_.filter(myRoom.room.memory.labs, lab => lab.resourceType == boost.resourceType).map(l => l.id)).sort((a, b) => (b.store.getUsedCapacity('energy') + b.store.getUsedCapacity(boost.resourceType)) - (a.store.getUsedCapacity('energy') + a.store.getUsedCapacity(boost.resourceType))); if (!labsObj.length) return
            const labId = labsObj[0].id; if (!labId) return
            const lab = _obj(labId); if (!lab) return
            if (lab.store.getUsedCapacity(boost.resourceType) >= boost.bodyPartsCount * 30 && lab.store.getUsedCapacity('energy') >= boost.bodyPartsCount * 20 && !this.pos.inRangeTo(lab, 1)) {
                this.travelTo(lab)
                this.say('boost')
                return true
            }
            else {
                if (this.memory.role == 'upgrade') return false
                this.say('w8 4 b');
                return true
            }
        }
...
Creep.prototype._heal = function () {
    ...
    if (range1.length) { this.heal(range1.sort((a, b) => a.hits - b.hits)[0]); return }
    else if (!this.memory.rangedAttack) {
        ...
        // else { this.heal(this); return true }
    }
    if (this.room.controller && this.room.controller.owner && !this.room.controller.my) { this.heal(this); return true }
    // const myRoom = myRooms[this.room.name]
    // if (myRoom && myRoom.enemies.length) {
    // const enemies = idToObj(myRoom.enemies.filter(e => cache.enemies[e.id].parameters.rangedAttack || cache.enemies[e.id].parameters.attack))
    if (this.pos.findInRange(FIND_HOSTILE_CREEPS, 3).length) { this.heal(this); return true }
    // }
...
Creep.prototype.rangedAttack3 = function () {
    ...
    // else if (structures) { if (structures.length) { structures.sort((a, b) => !tools.lookRampart(a.pos) - !tools.lookRampart(b.pos) && a.hits - b.hits); this.rangedAttack(structures[0]) } }
        ...
        if (structures.length) {
            let str
            str = structures.filter(s => !tools.lookRampart(s.pos) && s.structureType !== 'constructedWall')

            if (str.length) {
                if (str.filter(s => this.pos.inRangeTo(s, 1) && s.structureType !== 'road' && s.structureType !== 'constructedWall').length > 1) { this.rangedMassAttack() }
                else {
                    str.sort((a, b) => a.hits - b.hits); this.rangedAttack(str[0])
                }
            }
            else {
                str = structures.filter(s => this.pos.inRangeTo(s, 1) && s.structureType == 'rampart')
                if (str.length > 1) { this.rangedMassAttack() }
                else {
                    str = structures.filter(s => s.structureType == 'rampart' || s.structureType == 'constructedWall').sort((a, b) => a.hits - b.hits)
                    if (str.length) { this.rangedAttack(str[0]) }
                }
            }
        }
    }
