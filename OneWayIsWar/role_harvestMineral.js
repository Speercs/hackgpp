...
myRooms_tool.manageTaskHarvestMineral = function () {
    ...
    let maxHarvestMineral = settings.tasks.maxHarvestMineral
    if (settings.rotateRooms && maxHarvestMineral > settings.activeRooms) maxHarvestMineral = settings.activeRooms
    let freeWorkers = maxHarvestMineral - _.map(cache.myRooms, myRoom => myRoom.queues.spawn.filter(st => st.role == role).length).reduce((a, b) => a + b, 0) - _.filter(Game.creeps, creep => creep.memory.role == role).length
    if (freeWorkers > 0) {
        for (const myRoom of _.filter(cache.myRooms, mr => mr.tasks.harvestMineral.requiredWorkers)) {
            if (!myRoom.tasks.harvestMineral.amount) { myRooms_tool.updateTaskHarvestMineral; break }
