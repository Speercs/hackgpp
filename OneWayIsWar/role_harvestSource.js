...
MyRoom.prototype.harvestSource = function (sourceId) {
    ...
    creeps.forEach(creep => {
        ...
        if (creep.store.getCapacity() == null && task.container) {
            ...
            creep.harvest(_obj(sourceId)); return
        }
        ...
        switch (creep.memory.status) {
            case 'harvest':
                ...
                if (creep.pos.roomName == task.pos.roomName) {
                    if (creep.memory.droppedEnergy && _.filter(cache.enemies, e => e.pos.roomName == this.name).length == 0) {
                    ...
                    if (this.level < 6 && !creep.store.getUsedCapacity(RESOURCE_ENERGY) && this.droppedResources.length && _.filter(cache.enemies, e => e.pos.roomName == this.name).length == 0) {
...
Creep.prototype.fillEnergyToStructures = function () {
    ...
    if (this.room.name == this.memory.roomName) {
        const spawns = idToObj(myRooms[this.memory.roomName].structures.spawns).filter(s => s.store.getFreeCapacity(RESOURCE_ENERGY))
        const extensions = idToObj(myRooms[this.memory.roomName].structures.extensions).filter(s => s.store.getFreeCapacity(RESOURCE_ENERGY))
        const towers = idToObj(myRooms[this.memory.roomName].structures.towers).filter(s => s.store.getFreeCapacity(RESOURCE_ENERGY))
    ...
    else { this.travelTo(Game.rooms[this.memory.roomName].controller) }
...
MyRoom.prototype.manageTaskHarvestSource = function () {
    ...
    for (const task of tasks) {
        ...
        const creepBody = spawnLogick.getBody(role, this.room.energyCapacityAvailable, { id:task.id,link: task.link, roomName: this.name, requiredWorkers: task.requiredWorkers, container: task.container })
        const specifications = {
            role: role,
            priority: settings.spawn.priority.harvestSource,
            creepBody: creepBody,
            memory: {
                id: task.id,
                roomName: this.name,
