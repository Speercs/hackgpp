'use strict'
//2023.06.28
Creep.prototype.edgeOfTheRoom = function () {
    if (this.pos.x == 0 || this.pos.y == 0 || this.pos.x == 49 || this.pos.y == 49) return true
    else return false
}
Creep.prototype.attack1Range = function (owner = false) {
    if (this.memory.attack1Range) {
        const enemy = _obj()
        if (enemy) {
            if (this.pos.inRangeTo(enemy), 1) { this.attack(enemy); this.move(enemy); return true }
    ...
    const enemies1Range = []
    enemiesArray.forEach(e => { if (this.pos.inRangeTo(e, 1)) { enemies1Range.push(e) } })
    if (enemies1Range.length) { this.attack(enemies1Range[0]); this.move(enemies1Range[0]); this.memory.attack1Range = enemies1Range[0].id; return true }
}
...
Creep.prototype.goToDie = function () {
    if (this.getFreeStore()) {
        const res = this.suicide()
        if (res == OK) {
            if (this.memory.role == 'manage') { myRooms[this.memory.roomName].clearManageMicroTasks(this.name) }
            else if (this.memory.role == 'harvest deposit') { tasks[this.memory.task].creepsCounter++ }
            const myRoom = myRooms[this.pos.roomName]
            if (myRoom) {
                if (myRoom.my) { myRoom.registerSheduledTask('updateTombstones') }
            }
        }
    }
}
...
Creep.prototype.getBoosted = function () {
    ...
    if (myRoom.room.memory.labsBoostMode) {
        ...
        else if (this.memory.role !== 'repair' && this.memory.role !== 'upgrade' && myRoom.queues.boost.filter(b => b.creepName == this.name).length) {
...
Creep.prototype._heal = function () {
    ...
    if (range1.length) { this.heal(range1.sort((a, b) => a.hits - b.hits)[0]); return }
        ...
        if (range3.length) { this.rangedHeal(range3.sort((a, b) => a.hits - b.hits)[0]); return }
        else { this.heal(this) }
    }
}
