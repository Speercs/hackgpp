...
MyRoom.prototype.defend = function () {
    ...
    let enemies = this.enemies.filter(e => cache.enemies[e.id].parameters.dismantle || cache.enemies[e.id].parameters.attack).filter(e => !namesOfAllies.includes(e.owner.username))
    ...
    defend_attack.forEach(creep => {
        ...
        if (creep.memory.enemyId) {
            ...
            if (enemy) {
                const rampart = enemy.pos.findClosestByRange(FIND_STRUCTURES, { filter: str => str.structureType == STRUCTURE_RAMPART && str.hits > 100000 && !checkPos(str.pos, creep) })
                if (rampart) {
...
MyRoom.prototype.manageTaskDefend = function () {
    ...
    else if (this.underAttack) { task.roles['defend_attack'].amount = 1 }
...
MyRoom.prototype.towersLogick = function () {
    ...
    if (this.underAttack) {
        ...
        if (enemiesId.length && towers.filter(t => t.store['energy'] == 0).length == 0) {
            ...
            if (repairInRoom) {
                const ramparts = idToObj(this.structures.ramparts)
                let ramparts1Hit = ramparts.filter(str => str.hits == 1)
