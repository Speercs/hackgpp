    ...
    for (let order of orders) {
        ...
        if (buyAmount > 0) {
            ...
            if (returnCode === OK) {
                this.registerSheduledTask('update storage terminal')
                memoryManager.addToUpdateList(order.resourceType)
                console.log(`BUY: ${order.resourceType}, id: ${order.id}, from: ${order.roomName} => ${this.name}, amount: ${buyAmount}, price: ${order.price}, totalPrice: ${order.totalPrice}, transactionCost: ${transactionCost}, distance: ${order.distance}`)
                break
