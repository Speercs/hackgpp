...
MyRoom.prototype.upgrade = function () {
    ...
    if (creeps.filter(creep => !creep.store.getUsedCapacity(RESOURCE_ENERGY)).length) {
        ...
        if (!enemies.length && !controllerLinkWasAdd) {
            ...
            const towersWithEnergy = towers.filter(tower => tower.store.getUsedCapacity(RESOURCE_ENERGY))
    ...
    creeps.forEach(creep => {
        ...
        switch (creep.memory.status) {
            ...
            case 'upgrade':
                if (this.room.controller.level == 8 && this.room.controller.ticksToDowngrade == 200000 && !settings.tasks.alwaysUpgrade) { creep.memory.goToDie = true; creep.goToDie(); return }
...
MyRoom.prototype.manageTaskUpgrade = function () {
    ...
    if (this.level < 8 || (this.level == 8 && this.room.controller.ticksToDowngrade < 190000) || settings.tasks.alwaysUpgrade) {
        ...
        if (settings.tasks.alwaysUpgrade && !this.room.memory.status) { if (!task.requiredWorkers) { task.requiredWorkers = 1 } }
