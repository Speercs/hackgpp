...
module.exports.loop = function () {
    ...
    if (gameTime > settings.scriptLoadTime + 100 && gameTime < settings.scriptLoadTime + 100 + cache.countOfMyRooms) {
        const list = cache.myRooms.filter(mr => mr.level == 8 && !mr.base.length)
        if (list.length) {
            try { const myRoom = list[0]; myRoom.updateBaseTerritory(); myRoom.updateInBaseForTasks() } catch (error) { console.log(myRoom.name, "updateBase error", error, `\n${error.stack}`) }
        }
    }
    if (each10) {
    ...
    if (each10) {
        if (each100) {
            ...
            cache.myRooms.filter(myRoom => myRoom.status !== 'halted').forEach(myRoom => {
                try { myRoom.buyOrSellResources() } catch (error) { console.log(`${myRoomLog}`, 'buyOrSellResources', error + `\n${error.stack}`) }
            })
        }
    ...
    //swap    
    swapCreepPositions()
    if (Game.cpu.getUsed() < 20 && Game.cpu.bucket > 1000) { gl_towers.repairRamparts() }
    // end of tick:
