...
MyRoom.prototype.updateLabs = function () {
    const cpu = settings.writePerformanceLog ? Game.cpu.getUsed() : 0
    let labsList = idToObj(Object.keys(this.structures.labs))
...
MyRoom.prototype.updateLabReaction = function () { //choose reaction
    ...
    for (const resourceType of cache.resourcesSortedToChooseByLabs) {
        if (resourcesTier4.includes(resourceType) && cache.resources[resourceType] > countOfMyRooms * settings.store.resourcesTier4) { continue }
        if (resourcesTier3.includes(resourceType) && cache.resources[resourceType] > countOfMyRooms * settings.store.resourcesTier3) { continue }
        if (resourcesTier2.includes(resourceType) && cache.resources[resourceType] > countOfMyRooms * settings.store.resourcesTier2) { continue }
    ...
    if (task.reaction) {//clear mini tasks for labs
        ...
        delete this.lastUpdateMicroTasks_labs
    }
...
MyRoom.prototype.setLabsBoostMode = function (turnOff = false) {
    if (turnOff) {
    ...
    const boosts = ['XZHO2', 'XGHO2', 'XUH2O', 'XLHO2', 'XKHO2', 'XZH2O', 'XLH2O', 'XUHO2', 'XKH2O', 'XGH2O']
    this.room.memory.labs = {}
            ...
            if (level === 5) {
                ...
                if (!work) {
                    for (const res of lvl2) {
                        ...
                        if (debugFactory) { console.log('factory debug:', `res2:${res2}, this.resources[res2]:${this.resources[res2]}`) }
...
MyRoom.prototype.backupStructurePositions = function () {
    ...
    this.room.find(FIND_STRUCTURES, { filter: s => s.structureType !== STRUCTURE_CONTROLLER }).forEach(s =>
        this.room.memory.structurePositions.push({ type: s.structureType, pos: s.pos })
    )
    this.room.find(FIND_MY_CONSTRUCTION_SITES).forEach(s =>
        this.room.memory.structurePositions.push({ type: s.structureType, pos: s.pos })
    )
}
