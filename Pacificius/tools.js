...
tools.getContainersList = function (RoomName, type) {
    if (!Memory.rooms[RoomName].containers) { return false }
    let AllContainers = Object.values(Memory.rooms[RoomName].containers)
    let ContainersId = []
    Object.values(AllContainers).forEach((c) => {
        if (c.type == type) {
            ContainersId.push(c.id)
        }
    })

    if (ContainersId.length) {
        let containers = idToObj(ContainersId)
        return containers
    }
    return false
}
...
tools.updateAllyRooms = function () {
    //const ally = ['hackgpp', 'OneWayIsWar', 'agyacska', 'champion', 'Pacificius']
    const ally = [...namesOfAllies]
    ally.cut(settings.username)
    cache.roomsOfAlly = []
    ally.forEach(name => { cache.roomsOfAlly = cache.roomsOfAlly.concat(roomList[name]) })
}
